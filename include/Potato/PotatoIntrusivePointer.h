...
namespace Potato::Misc
{
	...
	struct DefaultIntrusiveObjectInterface
	{
		...
		void SubRef() { if(RefCount.SubRef()) delete static_cast<DriverT*>(this); }
	protected:
	...
	struct IntrusivePtr : private Wrapper
	{
		...
		IntrusivePtr(Type* t) : ptr(t) { if (t != nullptr) Wrapper::AddRef(ptr); }
		IntrusivePtr(IntrusivePtr&& ip) : ptr(ip.ptr) { ip.ptr = nullptr; }
		IntrusivePtr(const IntrusivePtr& ip) : IntrusivePtr(ip.ptr) {}
		~IntrusivePtr() noexcept { Reset(); }

		bool operator== (const IntrusivePtr& ip) const { return ptr == ip.ptr; }
		bool operator<(const IntrusivePtr& ip) const { return ptr < ip.ptr; }

		bool operator!= (const IntrusivePtr& ip) const { return !((*this) == ip); }
		bool operator<= (const IntrusivePtr& ip) const { return (*this) == ip || (*this) < ip; }
		bool operator> (const IntrusivePtr& ip) const { return !((*this) <= ip); }
		bool operator>= (const IntrusivePtr& ip) const { return !((*this) < ip); }
		...
		operator IntrusivePtr<require_type, Wrapper>() const
		{
		...
		operator IntrusivePtr<require_type, Wrapper>()
		{
			return static_cast<require_type*>(ptr);
		}

		IntrusivePtr& operator=(IntrusivePtr&& ip);
		IntrusivePtr& operator=(const IntrusivePtr& ip) {
			IntrusivePtr tem(ip);
		...
		*/
		...
		template<typename TargetType> TargetType* CastSafe() const { return static_cast<TargetType*>(ptr); }
		template<typename TargetType> TargetType* CastReinterpret() const { return reinterpret_cast<TargetType*>(ptr); }
		IntrusivePtr<std::remove_const_t<Type>, Wrapper> RemoveConstCast() const { return const_cast<std::remove_const_t<Type>*>(ptr); };
		Type* GetPointer() { return ptr; }
		Type const* GetPointer() const { return ptr; }
	private:
		friend Wrapper;
		Type* ptr;
	};
	...
	auto IntrusivePtr<Type, Wrapper>::operator=(IntrusivePtr&& ip) ->IntrusivePtr&
	{
