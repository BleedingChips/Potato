...
namespace Potato::Reg
{
	...
	struct CaptureWrapper
	{
		...
		auto Slice(std::u8string_view Source) const {
			assert(HasCapture());
			return decltype(Source){CurrentCapture->Slice(Source)};
		}
		auto Slice(std::wstring_view Source) const {
			assert(HasCapture());
			return decltype(Source){CurrentCapture->Slice(Source)};
		}
		auto Slice(std::u16string_view Source) const {
			assert(HasCapture());
			return decltype(Source){CurrentCapture->Slice(Source)};
		}
		auto Slice(std::u32string_view Source) const {
			assert(HasCapture());
			return decltype(Source){CurrentCapture->Slice(Source)};
		}
