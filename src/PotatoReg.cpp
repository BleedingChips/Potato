...
namespace Potato::Reg
{
	...
	EpsilonNFA CreateEpsilonNFAImp(std::basic_string_view<CharT> Str, bool IsRaw, Accept AcceptData)
	{
		...
		while (!Span.empty())
		{
			auto Re = StrEncode::CharEncoder<CharT, char32_t>::EncodeOnceUnSafe(Span, OutSpan);
			assert(Re);
	...
	namespace Exception
	{
		...
		{
			TotalString.resize(StrEncode::StrEncoder<char8_t, wchar_t>::RequireSpace(Str).TargetSpace);
			StrEncode::StrEncoder<char8_t, wchar_t>::EncodeUnSafe(Str, TotalString);
		}
		...
		UnaccaptableRegex::UnaccaptableRegex(TypeT Type, std::u16string_view Str, std::size_t BadOffset)
			: Type(Type), BadOffset(BadOffset)
		{
			TotalString.resize(StrEncode::StrEncoder<char16_t, wchar_t>::RequireSpace(Str).TargetSpace);
			StrEncode::StrEncoder<char16_t, wchar_t>::EncodeUnSafe(Str, TotalString);
		}

		UnaccaptableRegex::UnaccaptableRegex(TypeT Type, std::u32string_view Str, std::size_t BadOffset)
			: Type(Type), BadOffset(BadOffset)
		{
			TotalString.resize(StrEncode::StrEncoder<char32_t, wchar_t>::RequireSpace(Str).TargetSpace);
			StrEncode::StrEncoder<char32_t, wchar_t>::EncodeUnSafe(Str, TotalString);
		}
